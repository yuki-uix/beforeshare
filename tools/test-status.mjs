...
if (isMain) {
  ...
  // Read from the schema, not restated. A status added to the enum and missed here
  // would slip past the reachability check below without any failure.
  const VALID = JSON.parse(readFileSync(new URL('../schemas/v1/enums.schema.json', import.meta.url), 'utf8'))
    .$defs.status.enum;
