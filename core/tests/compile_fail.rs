...
fn forging_a_resolved_path_does_not_compile() {
    for (file, expected) in [
        ...
        (
            "tests/compile-fail/copy-a-read-path-into-a-write.rs",
            "E0451",
        ),
    ] {
        ...
        assert!(
            !built,
            "{file} produced a binary, so it compiled - and it must not"
        );
...
fn lib_path() -> String {
    ...
    newest
        .expect("the library is built before this test runs")
        .1
        .display()
        .to_string()
}
